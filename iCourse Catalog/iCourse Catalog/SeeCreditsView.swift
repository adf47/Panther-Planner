...
class SeeCreditsView: UIViewController, CLLocationManagerDelegate, MKMapViewDelegate,UICollectionViewDataSource, UICollectionViewDelegate {
   ...
   /* func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.bookmarkCred.tag = tagCount
        ...
        if(bookmarkArray.contains(self.classNumArray2[indexPath.row])){
            if let image = UIImage(named: "bookmark_pressed.png") {
                cell.bookmarkCred.setImage(image, for: .normal)
            }
        }
        
        //set up ratings here
        let stars = Int(arc4random_uniform(5) + 1)
        
        cell.rateCred.image = UIImage(named: "rating_\(stars)star.png")
    ...
    @IBAction func bookMarked(_ sender: Any) {
